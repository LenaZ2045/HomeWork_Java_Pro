package com.telran.org.homework12_10_2023;
...
public class SeaBattle {
    ...
    private static void startGame(char[][] playerFieldOne, char[][] playerFieldTwo, String playerOne, String playerTwo) {
        ...
        while (isPlayerAlive(playerFieldOne) && isPlayerAlive(playerFieldTwo)) {
            ...
            char[][] shotResult = verificationShot(currentPlayerEnemyField, currentPlayerField, xShot, yShot);
     ...
     */
    private static char[][] verificationShot(char[][] battleField, char[][] field, int x, int y) {
        ...
        for (int i = 0; i < field.length; i++) {
            battleField = field;
            if (x == 0 || y == 0) {
                System.out.println("Not all battles were shot yet, keep playing");
                printField(battleField);
                return battleField; // could not return zero as method needs to be void
            }
            if (x == 1 || y == 1) {
                System.out.println("No more battles - all battles got shot");
                fieldByDefault(field);
                fieldByDefault(battleField);
                return battleField; // and here unable to return 1 as method should be void
            }
        }
        return battleField;
    }
