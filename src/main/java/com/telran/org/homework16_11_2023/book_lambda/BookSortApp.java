...
// LAMBDA EXPRESSION FOR THE LINES FROM #39 THROUGH #47
        if (scanner.hasNext()) {
            bookList.forEach(System.out::println);
            System.out.println();
            bookList.sort(Comparator.comparing(Author::getAuthorName));
            System.out.println(bookList);
        }
...
// LAMBDA EXPRESSION FOR LINES FROM #66 THROUGH #74
        bookList1.forEach(System.out::println);
        System.out.println();
        bookList1.sort((Publisher book_1, Publisher book_2) -> {
            return book_1.getPublisher().compareTo(book_2.getPublisher());
        });
        System.out.println(bookList1);
    }
}
